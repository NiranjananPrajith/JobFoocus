...
// Local export helper — reads chrome.storage.local or localStorage
...
export async function bootstrapFromLocalStorage(userId: string): Promise<{ migrated: number }> {
  const supabase = getClient();

  // Already migrated? check Supabase first
  try {
  ...
  if (masterResume) {
    try {
      await dbSetMasterResume(userId, masterResume);
    } catch (e) {
  ...
  if (settings) {
    try {
      await dbSetSettings(userId, settings);
    } catch (e) {
