...
export default function AuthPage({ defaultTab }: { defaultTab: Tab }) {
  return (
    <Suspense fallback={<AuthFallback />}>
      <AuthContent defaultTab={defaultTab} />
    </Suspense>
  )
}
...
function AuthContent({ defaultTab }: { defaultTab: Tab }) {
  ...
  const handleTabChange = (newTab: Tab) => {
    ...
    setError(null)
    setSent(false)
    const path = newTab === 'login' ? '/login' : '/signup'
      ...
      <div className="w-full max-w-[400px]">
        <div className="text-center mb-8">
          ...
          <p className="text-[14px] text-steel mt-1">
            {isLogin ? 'Sign in to your Job Foocus account' : 'Start organizing your job search today'}
          </p>
        </div>
        ...
        {next && (
          <div className="mb-4 bg-primary/5 border border-primary/20 rounded-lg px-4 py-3 text-[13px] text-ink flex items-start gap-2">
            ...
            <span>
              ...
              {isLogin
                ? 'After signing in, you\'ll be taken straight back to finish adding the job you just scraped.'
                : 'After creating your account, you\'ll be taken straight back to finish adding the job you just scraped.'}
            </span>
          </div>
        )}
